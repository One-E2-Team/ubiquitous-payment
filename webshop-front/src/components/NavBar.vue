    ...
    <v-app-bar color="blue" dense>
      <v-container>
        <v-row align="center" justify="center">
          ...
          <v-col
            v-if="hasRole('SELLER')"
            @click="goToOrderedProducts()"
            cols="auto"
            ><v-btn>Ordered products</v-btn></v-col
          >
          <v-col
            v-if="hasRole('SELLER')"
            @click="goToPaymentOptions()"
            cols="auto"
            ><v-btn>Payment options</v-btn></v-col
          >
          <v-col v-if="hasRole('CUSTOMER')" @click="goToMyOrders()" cols="auto"
            ><v-btn>My orders</v-btn></v-col
          >
          <v-col cols="auto">
            <v-btn @click="logout()" v-if="isLogged()" class="mx-2"
