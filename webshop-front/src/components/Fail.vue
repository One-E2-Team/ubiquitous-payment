<template>
    <v-container>
          ...
          <v-col cols="12">
            <template>
                <div class="red accent-4 text-center">
                    <span class="white--text">Purchase failed</span>
                </div>
            </template>
          </v-col>
      </v-row>
      <v-row align="center" justify="center">
          <v-col cols="auto">
            <h2>Something went wrong! Your order with ID <b>{{pspId}}</b> failed!</h2>
          </v-col>
      </v-row>
      <v-row align="center" justify="center">
          <v-col cols="auto">
...
import * as comm from '../configuration/communication.js'
 export default {
    ...
    mounted(){
        ...
        axios({
                method: "put",
                url: comm.WSprotocol +'://' + comm.WSserver + '/api/psp-order/' + this.pspId + "/FAILED",
            }).then(response => {
